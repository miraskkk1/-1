...
// ИСПРАВЛЕННЫЙ ВЫЗОВ МОДЕРАЦИИ: Соответствует POST /api/sources/moderate и передаёт токен
    ...
    try {
        const response = await fetch(`${BACKEND_URL}/api/sources/moderate`, {
            ...
            headers: { 
                ...
                'Authorization': `Bearer ${token}` // Передаем токен для проверки сессии
            },
            body: JSON.stringify({ id: parseInt(id), status: status }) // Передаем id и status как ждет бэкенд
        });
