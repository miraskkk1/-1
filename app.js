...
async function loadPointsFromServer() {
    try {
        // Пробуем получить точки с сервера
        let response = await fetch(`${BACKEND_URL}/api/sources`);
        loadedSources = await response.json();
        
        // Если на бэкенде пусто (после перезапуска контейнера), синхронизируем базу генерацией из data.js
...
async function handleCreatePoint(event) {
    ...
    if (response.ok) {
        alert("Заявка успешно отправлена на карту на проверку модераторам!");
        closeAddPointModal();
        await loadPointsFromServer();
    }
}
...
async function moderatePoint(id, status) {
    ...
    if (response.ok) { await loadPointsFromServer(); }
}
    ...
    queueContainer.innerHTML = pendingPoints.map(p => `
        <div class="bg-white p-3 border rounded-lg flex justify-between items-start gap-4">
            <div>
                ...
                <p class="text-gray-500 text-[11px]">📍 Район: ${p.district}</p>
                <p class="text-gray-400 text-[10px]">Автор: ${p.author} | pH: ${p.ph}</p>
            </div>
...
async function checkBackendSession() {
    ...
    } catch(e) { localStorage.removeItem("jwtToken"); }
}
    ...
    detailsCard.innerHTML = `
        <div class="flex justify-between items-start gap-4 flex-wrap border-b pb-2 mb-3 text-xs">
            <div>
                ...
                <p class="text-gray-500 text-[11px]">📍 ${item.location}</p>
            </div>
            <div>${statusBadge}</div>
        </div>
